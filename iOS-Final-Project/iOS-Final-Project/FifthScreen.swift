...
class FifthScreen: UIViewController, UITextFieldDelegate {
         ...
         let address = tf1.text
             CLGeocoder().geocodeAddressString(address!, completionHandler: { placemarks, error in
                 ...
                 if let placemark1 = placemarks?[0]{
                     ...
                     let staticlocation = CLLocationCoordinate2D(latitude: Double.init(lat) ?? 0.0, longitude: Double.init(lon) ?? 0.0)
...
//                           )
                     ...
                     self.mv1.setRegion(region1, animated: true)
                     
                   let annotation = MKPointAnnotation()
                    annotation.coordinate = staticlocation
                     annotation.title = self.tf1.text
